...
def _quote_workflow(evidence_status: str) -> str:
    base = (
        ...
        "- Explain that if they want a quote WITH insurance, they must provide a "
        "**declared_value** (the amount to insure).\n"
        "- If the user explicitly asked for insurance but did NOT provide a "
        "declared_value: you MUST NOT generate a quote. Instead, tell the user "
        "that a declared value is required before quoting with insurance, and "
        "ask them to provide it.\n"
        "- If the user asked for insurance and DID provide a declared_value, "
        ...
        "- If the quote includes shipping, specify: origin, destination, carrier, "
        "and estimated delivery time if available.\n"
    )
